...
from bounce import Ball, Action
...
def mainLoop(player_count=3, player_colors=[], debug=False, CREATE_BALLS_MANUALLY=False, MAX_BALLS_IN_PLAY=300, CREATE_BALLS_COOLDOWN=30, BALLS_LEFT_TO_WIN=0):
    ...
    while True:
        ...
        if winnable and len(balls.keys()) <= BALLS_LEFT_TO_WIN:
            ...
            for x in player_balls:
                ...
                logging.info(f"Player {x}: {tick} ticks, {player_balls[x].win_count} wins, {player_balls[x].loss_count} losses, {player_balls[x].ball_radius} size, Kills: {player_balls[x].kill_count}")
